...
<Info>
  `mode` is the switch on its own — `spawn: {mode: dynamic}` with no `limits`
  block is valid and uses the default ceilings. See
  [Dynamic spawn](/docs/how-it-works/spawn) for the full schema.
</Info>
...
- [Dynamic spawn](/docs/how-it-works/spawn) - The `spawn:` config block and its limits
- [`start`](/reference/commands/start) - Launch services
- [`spawn`](/reference/commands/spawn) - Deprecated command reference
