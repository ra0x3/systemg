...
([`SG0019`](/reference/dialog/codes#sg0019)) so you never get the wrong
...
| `logs` (nothing) | refused — [`SG0019`](/reference/dialog/codes#sg0019) |
...
[`SG0021`](/reference/dialog/codes#sg0021). Combining `--supervisor` with a
selector is [`SG0020`](/reference/dialog/codes#sg0020).
</Note>
...
with [`SG0204`](/reference/dialog/codes#sg0204) rather than guessed at.
...
nor `--max-age` is refused with [`SG0017`](/reference/dialog/codes#sg0017).
</Warning>
...
$ sysg logs --supervisor --since 2h --grep SG0110
```

`--since`, `--until`, `--grep` and `--all` work on the supervisor log too. There
they keep whole log events, so a multi-line error stays together when any of its
lines matches `--grep`.
...
with `--kind`, the time bounds, and `--all`. With `--supervisor` it keeps whole
events instead, as described above.
...
- [`inspect`](/reference/commands/inspect) - View detailed metrics
