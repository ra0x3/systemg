...
Restart services, applying any manifest change on the way through.
...
[same-PID live re-execution](/docs/installation/upgrading) without restarting
...
the recycle is refused ([`SG0303`](/reference/dialog/codes#sg0303)) and the
...
| `-` | `--delta` | Bounce only the units the manifest changed, plus their dependents, instead of every declared unit |
...
before teardown, then applies it: added services start, removed services stop,
and every remaining unit is bounced. `restart` means restart — the scope is read
off what you asked for, never off the diff, because a rebuilt binary at an
unchanged path hashes identically and a diff-derived scope would leave that
process running behind a green exit code.

```sh
$ sysg restart --config production.yaml --delta
```

`--delta` asks for the narrow reconcile instead: added services start, removed
services stop, changed services restart, and unchanged services retain their
PIDs. A changed dependency also reevaluates its transitive dependents. A
`--delta` run that ends up bouncing nothing fails with
[`SG0304`](/reference/dialog/codes#sg0304) rather than reporting success.

<Info>
  To add a service to the primary project without bouncing the stack, register
  the manifest with `sysg start --config <file>`: a registration applies the same
  delta and never bounces what it did not change.
</Info>
...
removed from the manifest stop, and every remaining unit is bounced. You do not
...
returns [`SG0101`](/reference/dialog/codes#sg0101):

<pre>
  <code>
    {"error["}<a href="/reference/dialog/codes#sg0101">SG0101</a>{"]: cron unit `<name>` cannot be restarted directly\n"}
  </code>
</pre>
...
- [`stop`](/reference/commands/stop) - Stop services
- [Deployment strategies](/docs/how-it-works/configuration#deployment)
