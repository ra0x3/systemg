...
<Info>
  A cgroup that cannot be applied **fails the service** rather than starting it
  unconfined — no root supervisor, an unwritable controller file, a value the
  kernel rejects. A manifest that says a service is bounded and a service that
  is not bounded must never both be true. Sizes are written in the units the
  kernel expects, so `512M` is accepted here and converted; a value systemg
  cannot read is refused at load with [`SG0210`](/reference/dialog/codes#sg0210).
</Info>
