...
that was already running. Reached by `restart --reconcile` when the manifest
delta touched nothing but cron-managed or skip-gated units — a rebuilt binary at
an unchanged path produces an identical config hash and is invisible to the
diff. A bare `sysg restart` bounces every declared unit and does not hit this.
