...
A restart finished without bouncing a single unit, so every process is the one
that was already running.

`restart` reconciles: it applies the manifest delta and adopts unchanged units
rather than bouncing them, so adding one service does not take the stack down
and a completed one-shot is not re-run. The cost is that a rebuilt binary at an
unchanged path hashes identically and is invisible to the diff — so a deploy
whose only manifest change is to a cron-managed or skip-gated unit reconciles
successfully while every long-running process stays on the old binary.

This code turns that into a hard failure instead of a green exit code. Pass
`--all` to bounce every declared unit regardless of the diff, or `-s <service>`
to bounce one outright. Stopping a removed unit counts as work, and `start` or
`sysg add` never raise this — neither asked for a restart.
