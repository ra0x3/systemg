...
### [SG0001](/reference/dialog/codes#sg0001)

Catchall — a failure that has no more specific diagnosis yet.

### [SG0002](/reference/dialog/codes#sg0002)
...
### [SG0003](/reference/dialog/codes#sg0003)
...
### [SG0004](/reference/dialog/codes#sg0004)
...
### [SG0005](/reference/dialog/codes#sg0005)
...
### [SG0007](/reference/dialog/codes#sg0007)
...
### [SG0008](/reference/dialog/codes#sg0008)
...
[`SG0102`](/reference/dialog/codes#sg0102)–[`SG0110`](/reference/dialog/codes#sg0110)
instead.

### [SG0009](/reference/dialog/codes#sg0009)
...
### [SG0010](/reference/dialog/codes#sg0010)
...
### [SG0011](/reference/dialog/codes#sg0011)
...
### [SG0012](/reference/dialog/codes#sg0012)
...
### [SG0013](/reference/dialog/codes#sg0013)
...
### [SG0016](/reference/dialog/codes#sg0016)
...
### [SG0020](/reference/dialog/codes#sg0020)
...
### [SG0021](/reference/dialog/codes#sg0021)
...
### [SG0101](/reference/dialog/codes#sg0101)
...
### [SG0023](/reference/dialog/codes#sg0023)
...
### [SG0104](/reference/dialog/codes#sg0104)
...
### [SG0105](/reference/dialog/codes#sg0105)
...
### [SG0106](/reference/dialog/codes#sg0106)
...
([`SG0103`](/reference/dialog/codes#sg0103) pre-start failure,
[`SG0104`](/reference/dialog/codes#sg0104) health check,
[`SG0105`](/reference/dialog/codes#sg0105) port in use) is in that service's
log.

### [SG0107](/reference/dialog/codes#sg0107)
...
[`SG0111`](/reference/dialog/codes#sg0111) when the supervisor is answering
and still working, and
[`SG0205`](/reference/dialog/codes#sg0205) when it has stopped answering at
all. The code is kept so a published diagnostic is not withdrawn.

### [SG0108](/reference/dialog/codes#sg0108)
...
### [SG0109](/reference/dialog/codes#sg0109)
...
### [SG0110](/reference/dialog/codes#sg0110)
...
### [SG0111](/reference/dialog/codes#sg0111)
...
[`supervisor.xml`](/docs/how-it-works/configuration#supervisor-configuration), where
`0` waits indefinitely. `sysg status` shows what the operation is doing.

## Selectors, status &amp; stop

### [SG0006](/reference/dialog/codes#sg0006)
...
### [SG0208](/reference/dialog/codes#sg0208)

A manifest's includes form a cycle, so assembly was refused.

### [SG0209](/reference/dialog/codes#sg0209)
...
### [SG0302](/reference/dialog/codes#sg0302)

A restart ran but left one or more units short of their target state.

### [SG0303](/reference/dialog/codes#sg0303)

A supervisor recycle stopped the old daemon but the new one did not come up.

### [SG0304](/reference/dialog/codes#sg0304)

A restart finished without bouncing a single unit, so every process it left
running is the one that was already running.

`restart` bounces every declared unit, so this only follows a run that narrowed
itself: `--delta`, which applies the manifest delta and adopts unchanged units.
A rebuilt binary at an unchanged path hashes identically and is invisible to
that diff, so a `--delta` deploy whose only manifest change is to a cron-managed
or skip-gated unit reconciles successfully while every long-running process
stays on the old binary. A project with no bounceable unit at all — every unit
cron-managed or skip-gated — never raises this: nothing was owed a bounce.

This code turns that into a hard failure instead of a green exit code. Drop
`--delta` to bounce every declared unit, or use `-s <service>` to bounce one
outright. Removing a unit does NOT excuse it — stopping a dropped unit replaces
no running process, and a production deploy that removed one cron entry rode
that exemption to a green exit over a stale binary. The run is not rolled back
when this fires — added units started and removed units stopped before the
verdict; what did not happen is a bounce. `start` and `sysg add` never raise
this: neither asked for a restart.

### [SG0305](/reference/dialog/codes#sg0305)

Something replaced a unit's process while a start was still bringing it up, so
that start reached no verdict and reports none.

The start launched a process and then found a different one holding the unit, so
it stopped nothing on the way out: whatever runs now belongs to whoever replaced
it. Ordinary causes are a second restart issued while the first was still
waiting on readiness, and a `restart_policy` respawn after the launched process
died. The unit itself may well be healthy; run `sysg status` to see what is
running now, and `sysg logs -s <service>` for why the first process went away.

## Purge

### [SG0401](/reference/dialog/codes#sg0401)

A purge was refused because a live supervisor is still managing processes; stop
it or pass `--force`.

### [SG0402](/reference/dialog/codes#sg0402)

A purge removed some state but hit an IO error before finishing, so the on-disk
state may be partial.

### [SG0403](/reference/dialog/codes#sg0403)

A scoped purge named a project that has no state on disk; nothing was deleted.

### [SG0404](/reference/dialog/codes#sg0404)
...
### [SG0405](/reference/dialog/codes#sg0405)
...
### [SG0501](/reference/dialog/codes#sg0501)
...
### [SG0502](/reference/dialog/codes#sg0502)
...
[compatibility and recovery path](/docs/installation/upgrading#compatibility).

### [SG0503](/reference/dialog/codes#sg0503)
...
### [SG0504](/reference/dialog/codes#sg0504)
...
### [SG0505](/reference/dialog/codes#sg0505)
...
### [SG0601](/reference/dialog/codes#sg0601)
...
### [SG0602](/reference/dialog/codes#sg0602)
...
### [SG0603](/reference/dialog/codes#sg0603)
...
### [SG0604](/reference/dialog/codes#sg0604)
...
### [SG0605](/reference/dialog/codes#sg0605)
...
### [SG0701](/reference/dialog/codes#sg0701)
...
### [SG0702](/reference/dialog/codes#sg0702)
...
### [SG0703](/reference/dialog/codes#sg0703)
...
### [SG0704](/reference/dialog/codes#sg0704)
...
### [SG0712](/reference/dialog/codes#sg0712)
...
### [SG0713](/reference/dialog/codes#sg0713)
...
### [SG0714](/reference/dialog/codes#sg0714)
...
A seccomp filter could not be built, compiled, or applied. systemg refuses the
service rather than run it without the requested syscall filter.

### [SG0723](/reference/dialog/codes#sg0723)

`prctl(PR_SET_NO_NEW_PRIVS)` failed, so seccomp and Landlock cannot be applied
safely. The service is refused.

### [SG0724](/reference/dialog/codes#sg0724)
...
### [SG0725](/reference/dialog/codes#sg0725)
...
### [SG0726](/reference/dialog/codes#sg0726)
...
### [SG0727](/reference/dialog/codes#sg0727)
...
### [SG0741](/reference/dialog/codes#sg0741)

A resource limit (`nofile`, `nproc`, or `memlock`) could not be set.

### [SG0742](/reference/dialog/codes#sg0742)

The scheduling priority from `limits.nice` could not be set.

### [SG0743](/reference/dialog/codes#sg0743)

The CPU set from `limits.cpu_affinity` could not be applied — usually a CPU
index that does not exist on this machine.

### [SG0744](/reference/dialog/codes#sg0744)

The supplementary group list could not be installed. The service is refused:
starting it would leave it holding the supervisor's groups.

### [SG0745](/reference/dialog/codes#sg0745)

The primary group could not be switched to the configured `group`.

### [SG0746](/reference/dialog/codes#sg0746)
...
### [SG0747](/reference/dialog/codes#sg0747)
...
### [SG0748](/reference/dialog/codes#sg0748)

Capabilities could not be fully dropped, so the service would have kept more
privilege than it was configured with.

### [SG0731](/reference/dialog/codes#sg0731)

Reserved — kernel-assisted observation is required by configuration but
unavailable; start refused with the missing prerequisites as evidence.

### [SG0732](/reference/dialog/codes#sg0732)

Reserved — kernel-assisted observation unavailable; degraded to polling.

### [SG0733](/reference/dialog/codes#sg0733)

Reserved — kernel-assisted event loss; a `/proc` reconciliation pass would be
forced.

### [SG0705](/reference/dialog/codes#sg0705)
...
### [SG0706](/reference/dialog/codes#sg0706)

`install-boot` refused to write or activate a service-manager unit. Either the
request cannot produce a working unit — `--sys` under a user manager, `--run-as`
on a user-scope unit, a system-scope unit that names neither `--sys` nor an
account — or a file already sits at the install path without sysg's marker, in
which case it is left exactly as it is. It also refuses to write a unit for a
manager this machine does not run: on a Linux host booted by runit, s6, OpenRC
or a bare container entrypoint there is nothing to read a systemd unit, so the
diagnostic points at running `start --attached` under that manager instead.
When the unit was written and the manager's own activation command failed or
could not be run at all, the diagnostic reports that command and why.
