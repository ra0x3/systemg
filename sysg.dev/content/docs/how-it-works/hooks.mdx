...
exit, or a cron boundary skipped because the previous run was still going.
...
| `onerr` | Service exits unsuccessfully before or after readiness, or a cron boundary is skipped because the previous run is still going |
...
|----------|---------|-------|
| `SYSG_HOOK_EVENT` | every hook | `onstart`, `service_exit`, `cron_exit` or `cron_overlap` |
| `SYSG_EXIT_CODE` | `service_exit`, `cron_exit` | The exit code, unset when there was none (killed by a signal, or never started) |
| `SYSG_SCHEDULED_AT` | `cron_exit`, `cron_overlap` | The schedule boundary the run was for, RFC 3339 UTC |
