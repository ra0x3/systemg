...
def signal_handler(sig, frame):
    print(f"Worker {os.environ.get('WORKER_ID', 'unknown')}: Received signal {sig}, shutting down gracefully...")
    sys.exit(0)
...
while True:
    time.sleep(5)
    print(f"Worker {os.environ.get('WORKER_ID', 'unknown')}: Processing task at {time.time()}")