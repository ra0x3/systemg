...
`2`. Older `version: "1"` manifests are no longer accepted — bump them to `"2"`
(run [`sysg migrate`](/how-it-works/commands/migrate), which emits `version: "2"`,
and `sysg purge` before upgrading).
...
| `retries` | number | Maximum attempts before giving up (default `3`) |

<Note>
`attempt_timeout` bounds **one** probe, not the whole check — the total wait is
derived: `retries × (attempt_timeout + interval)`. There is no separate
total-timeout knob. A refused endpoint returns immediately, so a taken port
fails fast rather than blocking for the full budget. The failure carries a code
by cause: [`SG0022`](/how-it-works/dialog/codes#sg0022) (could not reach),
