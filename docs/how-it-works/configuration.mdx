...
<Info>
  ...
  deadlines are protocol invariants and are not operator settings.
</Info>
...
| `pre_start` | string | Command that must exit successfully before starting; its process tree is terminated after the configured supervisor command budget (SG0108) |
...
<Note>
  ...
  the `supervisor.xml` `pre_start_secs` budget is SG0108, and the service is not
  launched.
</Note>
