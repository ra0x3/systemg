...
By default `sysg logs` reads the full active `{service}.log`; pass `--lines N`
to limit it to the last N lines. `--all` concatenates the rotated backups
