...
- a service name that does not exist → `SG0202`,
- a bare `-s <name>` that matches a service in more than one project → `SG0006`
  (pass `-p` to choose),
- a `-p` that disagrees with a `project/service` prefix → `SG0201`.
</Note>
