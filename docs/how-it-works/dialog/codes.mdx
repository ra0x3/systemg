...
keeps answering, up to a 15-minute backstop — a deadlocked owner thread would
otherwise hang the CLI forever, since the socket stays responsive either way.

Raise or remove the backstop with `SYSG_COMMAND_TIMEOUT=<seconds>`, where `0`
waits indefinitely. `sysg status` shows what the operation is doing.
