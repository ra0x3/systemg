...
therefore auditable in the repository, not asserted in prose: if kernel mode
were exploitable, it would ruin the product — so the evidence lives where
you can check it.
...
Every release-candidate milestone of a root-facing phase ships an immutable
audit report under `audits/v*/` before it tags, with an external reviewer in
addition to maintainer self-review. Supply-chain gates run in CI: `cargo
audit`, `cargo deny`, and `cargo vet` (whose exemption baseline is frozen —
new dependencies must be vetted).

The canonical trust model for all of systemg remains
[Security](/security); this page covers what kernel mode adds.
