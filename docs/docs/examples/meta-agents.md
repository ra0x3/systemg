...
running agent spawns another, always include `--parent-pid "${SPAWN_PARENT_PID}"`
in the `sysg spawn` command so the cascade relationship is preserved and
children terminate with their owner.
