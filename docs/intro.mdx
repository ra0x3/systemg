...
Run `sysg start` and your entire stack is running. Run `sysg stop` and every
service shuts down cleanly.

This example is a single loose bundle — services with no project. To run several
independent stacks under one supervisor, group them under a `projects:` map; see
[Projects](/how-it-works/projects).
